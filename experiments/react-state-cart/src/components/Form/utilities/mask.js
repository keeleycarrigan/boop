export function characterLimit(limit = 0, allowWhiteSpace = false) {
  return (val = '') => {
...
export function creditCard (val = '') {
  const card = numbersOnly(val).match(/(\d{0,4})(\d{0,4})(\d{0,4})(\d{0,4})/);
...
export function phoneMask1 (val = '') {
  const phone = numbersOnly(val).match(/(\d{0,3})(\d{0,3})(\d{0,4})/);
...
export function ssn (val = '') {
  const phone = numbersOnly(val).match(/(\d{0,3})(\d{0,2})(\d{0,4})/);

  return `${phone[1]}${phone[2] && `-${phone[2]}`}${phone[3] && `-${phone[3]}`}`;
}

export function zipcodeMask(val = '') {
  const phone = numbersOnly(val).match(/(\d{0,5})(\d{0,4})/);
